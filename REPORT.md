...
- **Пары с существующими растрами Sentinel-2:** 6 пар (54.5 %).
- **Пары с полным отсутствием оптических данных:** 5 пар (45.5 %).

> **Эмпирическая проверка содержимого S2-растров:** все 12 поставляемых файлов `SENTINEL2_*.tif` содержат **0 валидных пикселей** во всех 8 каналах (значение `-999.0` = nodata). То есть оптические индексы в кейсе де-факто недоступны ни для одной пары, а не только для 5 «слепых». Это подтверждает вывод абляционного анализа: Mode 3 ≡ Mode 2 (см. Раздел 5).

В число «слепых» для оптики пар входят не только 3 контрольные пары межени 2018 года, но и **два крупнейших наводнения июля 2019 года** (Благовещенск и Константиновка). Более того, фактическая проверка показала, что **все** поставляемые S2-растры содержат нулевое число валидных пикселей, поэтому чисто оптическая схема продемонстрировала бы полный отказ на всём наборе, а не только на 45.5 % пар.
...
Порог ограничен физически допустимым коридором радиометрического отклика спокойной воды. Фактические границы берутся из `config.yaml` (`otsu_min_db = -22.0`, `otsu_max_db = -14.0`):
$$T_{calibrated} = \mathrm{clip}(T_{Otsu}, -22.0\text{ дБ}, -14.0\text{ дБ})$$
...
|---|---|---|---|---|---|---|---|
| 1 | **Mode 1:** Naive SAR Otsu alone (бейзлайн) | 0.0978 | 0.3148 | 0.3545 | 0.4513 | **0.2436** | Ложные радиотени на сопках, шум спекла |
| 2 | **Mode 2:** SAR Otsu + HAND $\le 25$ м / Slope $\le 5^\circ$ | 0.0003 | 0.3544 | 0.3636 | 0.0543 | **0.1514** | Избыточно жесткая фильтрация уреза воды |
| 3 | **Mode 3:** SAR + MSI fusion (где доступно) | 0.0003 | 0.3544 | 0.3636 | 0.0543 | **0.1514** | Все S2-растры без валидных пикселей; эффект нулевой |
| 4 | **Mode 4 (Final):** Full pipeline (+ Refined Lee 7x7 + Double-Bounce + MMU 25px + GSW) | **0.2793** | **0.4499** | **0.4070** | **0.3579** | **0.3529** | Прирост $Q_{flood}$ от Mode 2/3 (0.0003) до 0.2793 |

Проверка арифметики финального скора:
$$Score = 0.45 \cdot 0.2793 + 0.25 \cdot 0.4499 + 0.15 \cdot 0.4070 + 0.15 \cdot 0.3579 = 0.1257 + 0.1125 + 0.0611 + 0.0537 = \mathbf{0.3529}$$
...
1. **Динамика метрик по ступеням:**
   ...
   - **Mode 4 (Финальный комплекс):** Включение адаптивного фильтра Refined Lee $7\times7$, инверсного детектора double-bounce ($\Delta VH \ge 2.0$ дБ) и маски постоянной воды GSW восстанавливает целевую зону затопления: $Q_{flood}$ возрастает до **0.2793**, а точность водного зеркала пика $Q_{water\_peak}$ достигает **0.4499**, что дает наивысший итоговый результат **Score = 0.3529**.

2. **Анализ пространственного перекрытия (IoU и F1):**
   - В выгрузке `data/ablation_results.json` средний растровый IoU финального Mode 4 составляет **0.1270**, а F1 — **0.2110**. 
...
3. **Прозрачный экспериментальный факт: оптическое слияние (Mode 3) ≡ Mode 2:**
   - Сравнение выгрузок `data/ablation_results.json` (Mode 2 и Mode 3) показывает **полную побайтовую идентичность всех 33 строк детализации** попарных сравнений и идентичность метрик: $Score = 0.1514$, $Spec_{base} = 0.0543$.
...
- Финальная конфигурация (Mode 4) после устранения замечаний жюри (обрезка по полигону AOI, устранение -inf аномалий, MMU постобработка контуров продукта) достигает **$Spec_{base} = 0.3579$** на меженных парах и общего показателя сходимости площадей **$Score = 0.3529$** (при растровом IoU = 0.1270 и F1 = 0.2110).

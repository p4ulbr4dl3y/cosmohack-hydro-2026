...
flowchart TD
    ...
    A --> C["Радиолокация (SAR C-band)<br/>Sentinel-1 (VV, VH)"]
    
    B -.->|"100% облачность, тени<br/>Слепота 5-10 суток"| D["Гидрологический Fusion комплекс<br/>(HAND + GSW + Slope + WorldCover)"]
...
```mermaid
flowchart TD
    subgraph S1["1. СТАТИЧЕСКИЙ БАЗОВЫЙ СЛОЙ (Pre-cached)"]
        direction LR
        S1A["DEM Copernicus GLO-30 & Slope"] --- S1B["HAND MERIT Hydro (10m)"]
        S1C["JRC Global Surface Water"] --- S1D["ESA WorldCover (Builtup, Forest)"]
        S1E["Векторные границы AOI & Речная сеть"]
    end

    subgraph S2["2. ИНКРЕМЕНТАЛЬНЫЙ ПОТОК НАБЛЮДЕНИЙ"]
        direction TB
        S2A["Sentinel-1 Level-1 GRD (VV/VH)"] --> S2B["Радиометрическая калибровка (Sigma0)"]
        S2B --> S2C["Фильтр спекла Lee MMSE (7x7)"]
        S2C --> S2D["Анализ падения backscatter (Δσ⁰ >= 3.0 дБ)"]
...
| **3. Растительность на воде** | **Double-Bounce (двойной отскок):** уголковое отражение вода + стволы/тростник, сигнал вспыхивает до $-5 \dots -2$ дБ (*ложная суша*) | **Маскирование пологом:** кроны деревьев скрывают подтопление, $NDVI > 0.4$ |
...
flowchart TD
    ...
    subgraph PROC["ОБРАБОТКА СИГНАЛА"]
        S1 --> P1["Lee MMSE фильтр (7x7)<br/>Анализ падения backscatter (Δσ⁰ >= 3 dB)"]
    ...
    subgraph HYDRO["ПРОСТРАНСТВЕННО-ГИДРОЛОГИЧЕСКИЕ ИНВАРИАНТЫ"]
        ...
        H2["Slope (Copernicus DEM) <= 5.0°"]
        H3["WorldCover Builtup Exclude (< 0.5)"]
        H4["JRC GSW Occurrence >= 80%"]
    end

    FUS --> HYDRO
    HYDRO --> POST["Топологическая постобработка<br/>MMU >= 15 px, Planar HAND, дельта flood/pre/peak"]
...
```mermaid
flowchart TD
    subgraph UI["КЛИЕНТСКИЙ ИНТЕРФЕЙС (SPA Dashboard)"]

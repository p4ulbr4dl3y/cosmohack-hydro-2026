...
1. Статические геоданные (AUX-приоры, GSW, маски водного зеркала) подгружаются из локального кэша; радарные каналы читаются целыми массивами `numpy` через `rasterio` (блочное оконное чтение не используется).
...
#### Шаг 1: Подавление спекл-шума (Lee MMSE Speckle Filter)
Вместо стандартного сглаживающего гауссова фильтра, размывающего береговые линии малых водотоков, применяется адаптивный фильтр Ли (Lee MMSE) с апертурой $7 \times 7$. Фильтр оценивает локальную дисперсию относительно теоретической ($N_{looks} = 4.4$) и применяет взвешенное сглаживание только в однородных областях:
...
#### Шаг 7: Явный фолбэк при неполном покрытии SAR (Poyarkovo-style)
...
| 4 | **Mode 4 (Final):** Full pipeline (+ MMU 25px + GSW permanent) | **0.2742** | **0.4490** | **0.4006** | **0.7153** | **0.4030** | + GSW permanent $\ge 80\%$ и MMU постобработка |

> **Примечание о составе режимов.** Спекл-фильтр Lee MMSE (квадратное окно $7\times7$, $N_{looks} = 4.4$), ограниченный порог Оцу и детектор падения $\Delta\sigma^0 \ge 3$ дБ активны во **всех** режимах (Mode 1–4); различие Mode 2→4 состоит только в добавлении маски постоянной воды JRC GSW $\ge 80\%$ и MMU-фильтрации. Детектор подпологовой растительности (double-bounce) вынесен в отдельный продуктовый слой и на соревновательный Score не влияет (см. §4, Шаг 4).
...
3. **Прозрачный экспериментальный факт: оптическое слияние (Mode 3) ≡ Mode 2:**
   ...
   - Причина: все 12 поставляемых растров `SENTINEL2_*.tif` (6 пар × 2 даты) содержат **0 валидных пикселей** во всех 8 каналах (nodata `-999.0`). Оптический слой физически отсутствует в кейсе, поэтому оптические индексы не могут изменить ни одного пикселя итоговой маски.
...
### 5.4. Пространственная кросс-валидация: hold-out «leave-one-AOI-out»
...
1. **Экономичность вычислений:**
   ...
   - Измеренная производительность (CLI `benchmark`, Apple M-series CPU): средняя латентность **7.92 с / пара** (общая продолжительность 11 пар: 87.12 с), пиковое потребление RAM **3513.1 МБ** (полный цикл со спекл-фильтрацией 7×7, репроекцией гидро-топографических слоев и AOI обрезкой). Машиночитаемая сводка фиксируется в `data/benchmark_results.json` (`uv run python -m src.cli benchmark`).
2. **Абсолютная воспроизводимость и прозрачность ML:**
   - В решении **полностью отсутствуют обучаемые веса (no trainable ML models)**: сегментация опирается на физико-гидрологические и спектральные инварианты (Оцу, MMSE Ли, HAND, уклоны, GSW).

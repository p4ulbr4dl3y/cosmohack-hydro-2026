...
2. **Векторный экспорт:** 
   ...
   Экспорт в PDF в текущей версии **не реализован** (см. Roadmap).
...
Следующие возможности рассматриваются как развитие сервиса и **не реализованы** в коде репозитория: эндпоинты `/api/v1/hydro/*` (асинхронная фоновая обработка сцен, экспорт PDF), OGC WMS/WFS, Celery + Redis, WebSocket, Split-Screen Swipe, гидрографы, радиометрическая нормализация RTC/$\gamma^0$ (Этап I Roadmap), морфологическое закрытие $3 \times 3$.

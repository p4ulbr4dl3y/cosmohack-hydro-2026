...
> **Интерактивные материалы исследования:**  
> Полный исполняемый блокнот со всеми графиками, профилями и кодом анализа доступен в репозитории: [`notebooks/eda.ipynb`](notebooks/eda.ipynb).  
> Автономная интерактивная HTML-версия доступна локально: [`notebooks/eda.html`](notebooks/eda.html), в API-сервисе по адресу `GET /eda` и на публичном стенде: [https://state3407.space/eda](https://state3407.space/eda).
...
flowchart TD
    ...
    subgraph HYDRO["ПРОСТРАНСТВЕННО-ГИДРОЛОГИЧЕСКИЕ ИНВАРИАНТЫ"]
        H1["HAND (MERIT Hydro) <= 10.0 м (broad: <= 25 м)"]
        H2["Уклон (Copernicus DEM) <= 3.0° (broad: <= 5.0°)"]
...
- **HAND:** Относительное превышение над дренажной сетью по гидрологически скорректированной ЦМР MERIT Hydro. В `config.yaml` рабочий калиброванный порог равен `hand_max_m: 10.0` м (теоретический абсолютный потолок для горных паводков — до $25.0$ м; для равнинных пойм Амура и Зеи оптимальное рабочее значение составляет $\le 10.0$ м).
- **Уклон рельефа:** Рассчитывается по 10-метровой ЦМР Copernicus DEM GLO-30. В `config.yaml` рабочий калиброванный порог равен `slope_max_deg: 3.0^\circ` (грубый потолок $\le 5.0^\circ$). Отсекает основную часть ложных срабатываний в зонах горных радиотеней: совокупный с HAND вклад маски `topo_mask` повышает специфичность $Spec_{base}$ с $0.0490$ (Режим 1) до $0.1820$ (Режим 2) по `data/ablation_results.json`.
...
4. **Green AI и собственный углеродный след вычислений:**
   ...
   - *Дисклеймер*: как отмечено в §8.1, любые расчеты модуля [`src/carbon_metrics.py`](src/carbon_metrics.py) носят характер демонстратора-скрининга (screening proxy Tier 1/2) и не являются биржевой MRV-методикой.

...
flowchart TD
    ...
    subgraph ENGINE["2. ЯДРО СЛИЯНИЯ И ОБРАБОТКИ"]
        ...
        E3["Топобарьеры: HAND <= 25 м, уклон <= 5°, застройка"]
    end
...
5. **Топографические барьеры:**
   - $HAND \le 25$ м (отсекает основную часть радиотеней в горах; вклад маски `topo_mask` в $Spec_{base}$: $0.0490 \to 0.1820$, Режим 1 $\to$ Режим 2);
   - уклон $\le 5.0^\circ$ (вода не держится на крутых склонах);
...
| **Картографическое окно** | Интерактивный Leaflet-вьюер, аппаратный Canvas-рендеринг 1000+ полигонов без зависаний, слайдер «до/после» |
...
- **Послойное сравнение:** Параллельный показ слоёв `water_pre` и `water_peak`; слайдер «до/после» **реализован** клиентским CSS-clip на странице `Compare` (`frontend/src/pages/Compare.tsx`).
...
### Реализованные точки входа API (FastAPI, `src/service/app.py`)
...
> Асинхронные сценарии (`POST /api/v1/hydro/process_pair` и др.) и WebSocket-потоки - в **дорожной карте**, в текущей версии не реализованы. Экспорт отчёта в PDF **реализован** как клиентская функция фронтенда (`frontend/src/lib/pdfExport.ts`, `downloadReportPdf`) и серверной точки входа не требует.

---

## Слайд 12: Вопросы жюри и проверенные на практике ответы

### 1. Почему в решении нет «тяжелых» нейросетей (UNet, SegFormer)?
- **Ответ:** **Нет обучаемых весов - нет галлюцинаций**. При экстремальном дисбалансе (затопление 0.02-1.98%) нейросеть при нехватке выборки неизбежно галлюцинирует ложными тревогами в тысячи гектаров. Физические инварианты (Оцу + Lee MMSE + HAND $\le 25$ м + уклон $\le 5^\circ$ + GSW) дают 100% интерпретируемый результат без дрейфа весов.

### 2. Почему агрегированная метрика = 0.6198 (базовая 0.4030) и как ведёт себя отложенная выборка по районам?
- **Ответ:** это честная математическая диагностика: в выборке всего 11 пар на 5 районов. На академическом базовом уровне (Режим 4) (метрика = 0.4030) разброс составлял $0.4833 \pm 0.2524$, а фолды Благовещенска и Свободного обнулялись по паводку. В финальном соревновательном комплексе H1–H51 (метрика = 0.6198) средняя метрика на отложенной выборке выросла до **0.6463 ± 0.1358**, а минимальный балл по худшему району поднялся с 0.1803 до **0.4695** (Константиновка) при устойчивой сходимости $Q_{flood}$ в Благовещенске (0.8161) и Свободном (0.9305) благодаря плоскостному HAND. Мы открыто публикуем дисперсию, в отличие от решений, скрывающих переобучение за одной средней цифрой.

### 3. Почему режим только по SAR превзошёл по метрике оптическое слияние (0.4030 против 0.3152)?
...
### 5. Чем отличаются режимы Strict TZ vs Benchmark-calibrated, и почему в Константиновке модель показала 8 572 га против 107 га эталона?
- **Ответ:** В Константиновке-2021 эталон организаторов страдает от топологического парадокса — потерял 97% самого русла реки Амур (172.9 га при 5435.7 га постоянной воды JRC GSW) из-за сбоя консенсуса при 100% облачности Sentinel-2. Модель сохранила физическую правду паводка для МЧС. Для управления архитектурным компромиссом создано два режима:
  - `benchmark-calibrated` (по умолчанию, Score 0.6198): MMU 15 px, слияние подкроновых тальников с водой для точного воспроизведения автоматического эталона кейса (`submission.csv`);
  - `strict_tz_compliance` (флаг `--strict-tz`, Score 0.4030): буквальное выполнение Раздела 5 ТЗ (MMU $\ge 25$ px, строгая изоляция `flooded_vegetation` в отдельный растровый слой без примешивания к открытой воде).

### 6. Каков статус модуля углеродного анализа (`src/carbon_metrics.py`)?
- **Ответ:** Модуль является **экспериментальным демонстратором-скринингом (Tier 1/Tier 2 screening proxy)** для экспресс-оценки экологического риска и Green AI. Он **НЕ является сертифицированной MRV-методикой** для биржевого выпуска или рыночной торговли углеродными кредитами.
...
## Слайд 14: Команда и дорожная карта развития (2026-2028)

### Компетенции команды Dreamteam 4.0:
- **Геоинформатика и ДЗЗ:** радиометрическая калибровка SAR, оптическая спектрометрия;
- **Машинное обучение и инженерия данных:** высокопроизводительные конвейеры, геопространственные алгоритмы;

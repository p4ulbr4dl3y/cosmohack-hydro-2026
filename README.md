...
#### 4.2. Экспериментальное исследование методов повышения скора (0.4030 → 0.5708)

В ходе исследовательской оптимизации архитектуры было последовательно выдвинуто и протестировано **17 инженерных гипотез** в четырех раундах с использованием изолированных сред валидации:
...
| **13. Стеблевой RVI-фильтр (H14)** | Радарный индекс растительности $RVI = 4\sigma^0_{VH}/(\sigma^0_{VV}+\sigma^0_{VH}) \le 0.20$: разделение открытой воды и стеблей сои. | Надежно сепарирует всходы в пойме, скор 0.5708 стабилен, 100% тестов пройдено. | **Исследовано** |
| **14. Коэффициент деполяризации (H15)** | Radar Depolarization Ratio $DPR = \sigma^0_{VH} - \sigma^0_{VV} \le -8.0$ дБ: вода зеркальна, а влажная шероховатая пашня деполяризует луч. | Подтверждено физическое разделение; скор 0.5708 сохранен. | **Исследовано** |
| **15. RVI Water Gate (H16)** | Физический барьер $RVI \le 0.20$ для подтверждения водного зеркала без оптических каналов. | Сохраняет открытую воду (Благовещенск 464 га), блокирует летнюю растительность на межени, скор 0.5708. | **Исследовано** |
...
| **17. Финальный оптимум ансамбля** | Полный калиброванный ансамбль: би-поляризация + топобарьер + multi-seed гидросвязность + радиометрический потолок. | **Финальный соревновательный Score: 0.5708** ($Q_{flood}=0.5577$, $Q_{peak}=0.4612$, $Q_{pre}=0.3948$, $Spec_{base}=0.9684$). | **Внедрено** |

...
2. **Группа Б - Исключенное постоянное русло (Благовещенск 2021, Свободный 2019, Константиновка 2021):**
   ...
   - **Физический парадокс метрики:** Паводок $flood$ на этих парах модель определяет с высокой точностью (**Благовещенск 2021: $Q_{flood} = \mathbf{0.88}$**, **Свободный 2019: $Q_{flood} = \mathbf{0.88}$**). Однако компоненты $Q_{water\_peak}$ и $Q_{water\_pre}$ обращаются в $0.0$, поскольку физически корректный детектор видит реальное полноводное русло реки Амур и Зея, а в эталоне оно искусственно обнулено.
...
| :--- | :---: | :---: | :---: | :---: | :---: | :---: | :--- |
| **Благовещенск 2021** | 882.6 | 780.6 | **0.88** | 2845.2 | 12814.6 | 7247.4 | Русло Амура вырезано из эталона $water\_peak$ |
| **Свободный 2019** | 213.3 | 187.7 | **0.88** | 991.5 | 5070.6 | 3871.6 | Русло Зеи вырезано из эталона $water\_peak$ |
| **Константиновка 2021** | 107.3 | 8572.1 | 0.00 | 172.9 | 22388.1 | 5435.7 | Сверхвлажная соевая пойма + вырезанное русло |

*(Примечание: площади модели строго соответствуют финальному сабмиту [`submission.csv`](submission.csv) и отчету `data/final_evaluation_results.json`).*
...
- **Принципиальное инженерное решение команды:** в сабмите [`submission.csv`](submission.csv) сохранен **рекордный физически калиброванный ансамбль (Score = 0.6198)** без ручной подгонки под неоднородные соглашения разметчиков, что гарантирует полную физическую состоятельность и устойчивость системы на реальных некорректированных данных МЧС.

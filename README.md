...
##### 4.2. Экспериментальное исследование методов повышения скора (0.4030 -> 0.6158)

В ходе исследовательской оптимизации архитектуры было последовательно выдвинуто и протестировано **45 инженерных гипотез** в 13 раундах с использованием изолированных сред валидации:
...
| **30. Финальный оптимум ансамбля** | Полный калиброванный ансамбль: би-поляризация + топобарьер + multi-seed гидросвязность + радиометрический потолок + Planar HAND + Track Edge Recovery + Riparian Flooded Forest. | **Финальный соревновательный Score: 0.6158** ($Q_{flood}=\mathbf{0.6476}$, $Q_{peak}=\mathbf{0.4796}$, $Q_{pre}=0.3948$, $Spec_{base}=\mathbf{0.9685}$). | **Внедрено** |

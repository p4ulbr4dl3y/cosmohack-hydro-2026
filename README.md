...
### 4.2. Экспериментальное исследование методов повышения скора (0.4030 → 0.5452)

В ходе исследовательской оптимизации архитектуры было последовательно выдвинуто и протестировано **8 инженерных гипотез** в двух раундах с использованием изолированных сред валидации:
...
| **7. Morphological Closing** | Затягивание микродыр внутри водных зеркал ядром $5\times5$ disk: устранение спекл-шума в руслах. | Стабилизирует площади водного зеркала ($Q_{water\_peak}$ вырос до 0.4605). | **Принято** |
| **8. Интеграция ансамбля мер** | Объединение всех победивших приемов (Dual-Pol + Multi-Seed + Closing + Calibrated Thresholds). | **Финальный Composite Score: 0.5452 (+35.3% прирост к базе 0.4030)**. | **Внедрено** |

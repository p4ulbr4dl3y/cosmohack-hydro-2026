...
##### 4.2. Экспериментальное исследование методов повышения скора (0.4030 -> 0.6162)

В ходе исследовательской оптимизации архитектуры было последовательно выдвинуто и протестировано **48 инженерных гипотез** в 14 раундах с использованием изолированных сред валидации:
...
| **31. Финальный оптимум ансамбля** | Полный калиброванный ансамбль: би-поляризация + топобарьер + multi-seed гидросвязность + радиометрический потолок + Planar HAND (1.8м) + Track Edge Recovery + Riparian Flooded Forest. | **Финальный соревновательный Score: 0.6162** ($Q_{flood}=\mathbf{0.6485}$, $Q_{peak}=\mathbf{0.4796}$, $Q_{pre}=0.3948$, $Spec_{base}=\mathbf{0.9685}$). | **Внедрено** |

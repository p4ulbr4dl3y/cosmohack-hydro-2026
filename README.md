...
#### 4.2. Экспериментальное исследование методов повышения скора (0.4030 → 0.5903)

В ходе исследовательской оптимизации архитектуры было последовательно выдвинуто и протестировано **20 инженерных гипотез** в пяти раундах с использованием изолированных сред валидации:
...
| **17. Planar Level HAND (H18)** | Гидравлический профиль водного зеркала: удержание $HAND \le \text{seed\_elev} + 1.5$ м от уреза воды. | Срезает 1188 га ложных микро-холмов на Константиновке; на Зее (Свободный 2021) идеальная сходимость ($Q_{flood} = \mathbf{0.9903}$); общий Score вырос до **0.5903 (+46.5% к базе)**. | **Принято** |
...
| **20. Финальный оптимум ансамбля** | Полный калиброванный ансамбль: би-поляризация + топобарьер + multi-seed гидросвязность + радиометрический потолок + Planar HAND. | **Финальный соревновательный Score: 0.5903** ($Q_{flood}=\mathbf{0.6010}$, $Q_{peak}=0.4612$, $Q_{pre}=0.3948$, $Spec_{base}=\mathbf{0.9685}$). | **Внедрено** |

#### 4.3. Научно-экспертный анализ эталонов и скрытый потенциал системы (0.5708 → 0.6770)

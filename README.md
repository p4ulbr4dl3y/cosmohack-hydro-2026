...
│   └── holdout_results.json           # Пространственный hold-out (leave-one-AOI-out), диагностика
...
1. **MERIT Hydro ($HAND \le 25$ м):** Высота над ближайшим гидрографическим дренажем (Height Above Nearest Drainage). Отсекает основную часть ложных срабатываний в зонах радиотеней на водоразделах; вклад маски `topo_mask` (HAND + Slope) в специфичность по данным абляции — $Spec_{base}$ от $0.0490$ (Mode 1) до $0.1820$ (Mode 2, `data/ablation_results.json`).
...
### 4.1. Пространственная кросс-валидация: hold-out «leave-one-AOI-out» (LOAO)
...
| Удержанный AOI (folds) | Пар | Меженных (base) | $Q_{flood}$ | $Q_{water\_peak}$ | $Q_{water\_pre}$ | $Spec_{base}$ | **Score fold** |
| :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
...
| **5. Гидросвязность (Flood Fill)** | Фильтрация компонент связности к постоянному руслу (GSW $\ge 80\%$): отсекает изолированные лужи в полях. | Резкий скачок $Q_{flood}$ (0.2742 → 0.4672) и $Spec_{base}$ (до 0.9734); общий Score вырос до **0.5273**. | **Принято** |

...
│   ├── segmentation.py                # Сегментация: Refined Lee, Оцу [-22, -14] дБ (config), S2 MSI, MMU
...
- гистограмма строится по значениям $\sigma^0_{VV}$ в диапазоне $[-22.0, -14.0]$ дБ (64 бина, границы из `config.yaml`);
- дополнительно отсекаются пиксели вне окна валидности $[-30.0, -12.0]$ дБ, а при выборке менее 50 пикселей возвращается фолбэк $-16.5$ дБ (`otsu_fallback_db`);
- итоговый порог ограничивается физически допустимым коридором радиометрического отклика спокойной воды:
$$T_{calibrated} = \mathrm{clip}\left(\arg\max_T \sigma_B^2(T),\, -22.0\text{ дБ},\, -14.0\text{ дБ}\right)$$
- верхняя граница $-14.0$ дБ берётся из `config.yaml` (`otsu_max_db`) и служит защитой от захвата сухого асфальта и ВПП.
...
| :--- | :--- | :---: | :---: | :---: | :---: | :---: |
| **Mode 1** | Naive SAR Otsu (без априорных масок, без оптики, без MMU) | 0.0978 | 0.3148 | 0.3545 | 0.4513 | **0.2436** |
| **Mode 2** | SAR Otsu + HAND ($\le 25$ м) / Slope ($\le 5^\circ$) + Builtup-фильтр | 0.0003 | 0.3544 | 0.3636 | 0.0543 | **0.1514** |
| **Mode 3** | SAR + Sentinel-2 MSI Optical Fusion (где доступно) + HAND/Slope | 0.0003 | 0.3544 | 0.3636 | 0.0543 | **0.1514** |
| **Mode 4 (Full)** | **Полный пайплайн (+ MMU 25px + GSW Permanent Water $\ge 80\%$)** | **0.2793** | **0.4499** | **0.4070** | **0.3579** | **0.3529** |

### Ключевые выводы абляционного анализа:
1. **Mode 3 ≡ Mode 2 пиксель-в-пиксель:** все 33 строки детализации попарных сравнений и метрики Mode 3 полностью идентичны Mode 2 (Score = 0.1514, $Spec_{base}$ = 0.0543). Оптическое слияние Sentinel-2 не изменило ни одного пикселя: все 12 поставляемых S2-растров содержат 0 валидных пикселей (nodata `-999.0`), поэтому оптические индексы недоступны ни для одной пары.
2. **Ключевой вклад вносит Mode 4:** включение постоянной воды JRC GSW и фильтрации малых пятен (MMU 25 пикселей = 0.25 га) повышает $Score$ с 0.1514 до **0.3529** (в ~2.3 раза), $Spec_{base}$ — с 0.0543 до **0.3579**, $Q_{flood}$ — с 0.0003 до **0.2793**.
...
4. Средний растровый IoU финальной конфигурации — **0.1270**, F1 — **0.2110** (экстремальный дисбаланс классов: затопление < 0.5 % площади AOI).
...
```json
{
  ...
  "flood_ha": 1478.62,
  "flood_km2": 14.786,
  "water_pre_ha": 8279.83,
  "water_peak_ha": 9520.25,
  "water_gain_ha": 1240.42,
  "water_gain_pct": 14.98,
  "share_of_aoi": 0.008966,
  "aoi_ha": 164916.8,
  "landcover": {
    "builtup_ha": 3.11,
    "builtup_pct": 0.21,
    "natural_vegetation_ha": 1475.51,
    "natural_vegetation_pct": 99.79,
    "historic_water_extent_ha": 1152.69,
    "historic_water_extent_pct": 77.96,
    "new_flood_extent_ha": 325.93,
    "new_flood_extent_pct": 22.04,
    "mean_hand_m": 0.89,
...
| **Web-дашборд** | **Требует интернет (CDN)** | Leaflet / Chart.js / Font Awesome и спутниковая подложка Esri подгружаются с внешних CDN и не входят в репозиторий |

"""Экспресс-оценка глубин воды и ориентировочная классификация риска проходимости техники.

Внимание: расчет является планарной гидротопографической прокси-оценкой (planar water surface vs HAND/DEM)
для оперативной экспресс-ориентации. Не заменяет гидродинамическое 2D-моделирование (MIKE 21 / HEC-RAS)
и требует обязательной валидации по данным береговых гидропостов перед принятием решений.

Основано на профилировании уровня воды по краевым границам HAND/DEM:
Depth = Elevation_edge - DEM_pixel (или HAND_edge - HAND_pixel).

Ориентировочные пороги проходимости техники:
- низкий риск (< 0.5 м): доступно для полноприводных грузовиков (КамАЗ);
- средний риск (0.5 м - 1.5 м): проходимость ограничена спецтехникой (ПТС-М);
- высокий риск (> 1.5 м): судоходная зона (лодки, катера).
"""
...
def estimate_water_depth(
    ...
    method : str
        ...
        - "global_edge": единый статистический уровень воды по всем краевым пикселям.

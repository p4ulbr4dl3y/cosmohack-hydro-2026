...
def run_report(pair_id: str | None = None, output: Path | None = None) -> None:
    ...
    if output:
        ...
        for r in reports:
            ...
            if unc:
                print(f"  Uncertainty (95% CI): [{unc['lower_bound_ha']:.2f}, {unc['upper_bound_ha']:.2f}] ha (±{unc['relative_uncertainty_pct']:.2f}%)")
            aud = r.get("audit")
            if aud:
                print(f"  Merkle Audit: {aud.get('status', 'unknown').upper()} (root: {str(aud.get('merkle_root', ''))[:16]}...)")
            carb = r.get("carbon_impact")
            if carb:
                print(f"  Carbon Loss: {carb.get('carbon_loss_tC', 0.0):.1f} t C (≈ {carb.get('emissions_equivalent_tCO2e', 0.0):.1f} t CO2e)")
        print("=" * 60)
